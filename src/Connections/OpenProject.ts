...
import { BridgeConfigOpenProject, OpenProjectInstance } from "../config/Config";
import { IOpenProjectMergeRequest, IOpenProjectProject, IOpenProjectUser, IOpenProjectWebhookMREvent, IOpenProjectWebhookNoteEvent, IOpenProjectWebhookPushEvent, IOpenProjectWebhookReleaseEvent, IOpenProjectWebhookTagPushEvent, IOpenProjectWebhookWikiPageEvent } from "../OpenProject/WebhookTypes";
...
import { AccessLevel, SerializedOpenProjectDiscussionThreads } from "../OpenProject/Types";
...
interface ConnectionStateValidated extends OpenProjectConnectionState {
    ignoreHooks: undefined,
    enableHooks: AllowedEventsNames[],
}


export interface OpenProjectConnectionInstanceTarget {
    name: string;
}
export interface OpenProjectConnectionProjectTarget {
...
export type OpenProjectConnectionTarget = OpenProjectConnectionInstanceTarget|OpenProjectConnectionProjectTarget;
...
} as JSONSchemaType<OpenProjectConnectionState>;

export interface OpenProjectTargetFilter {
    instance?: string;
    parent?: string;
    search?: string;
}
...
/**
 * Handles rooms connected to a OpenProject repo.
 */
@Connection
export class OpenProjectConnection extends CommandConnection<OpenProjectConnectionState, ConnectionStateValidated> implements IConnection {
    static readonly CanonicalEventType = "uk.half-shot.matrix-hookshot.openproject.repository";
    static readonly LegacyCanonicalEventType = "uk.half-shot.matrix-github.openproject.repository";
    ...
    static ServiceCategory = "OpenProject";
    ...
    static async createConnectionForState(roomId: string, event: StateEvent<Record<string, unknown>>, {as, intent, storage, tokenStore, config}: InstantiateConnectionOpts) {
        ...
        const instance = config.openproject.instances[state.instance];
        if (!instance) {
            throw Error('Instance name not recognised');
        }

        const connection = new OpenProjectConnection(roomId, event.stateKey, as, config.openproject, intent, state, tokenStore, instance, storage);

        const discussionThreads = await storage.getOpenProjectDiscussionThreads(connection.connectionId);
    ...
    public static async assertUserHasAccessToProject(
        ...
        tokenStore: UserTokenStore, config: BridgeConfigOpenProject
    ) {
        ...
        const client = await tokenStore.getOpenProjectForUser(requester, instance.url);
        if (!client) {
    ...
    public static async provisionConnection(
        ...
        const OpenProjectConfig = config.openproject;
        const instance = OpenProjectConfig.instances[validData.instance];
        if (!instance) {
            throw Error(`provisionConnection provided an instanceName of ${validData.instance} but the instance does not exist`);
        }
        const permissionLevel = await this.assertUserHasAccessToProject(validData.instance, validData.path, requester, tokenStore, OpenProjectConfig);
        const client = await tokenStore.getOpenProjectForUser(requester, instance.url);
        if (!client) {
            throw new ApiError("User is not authenticated with OpenProject", ErrCode.ForbiddenUser);
        }
        ...
        const connection = new OpenProjectConnection(roomId, stateEventKey, as, OpenProjectConfig, intent, validData, tokenStore, instance, storage);

        const existingConnections = getAllConnectionsOfType(OpenProjectConnection);
        const existing = existingConnections.find(c => c.roomId === roomId && c.instance.url === connection.instance.url && c.path === connection.path);

        if (existing) {
            throw new ApiError("A OpenProject repo connection for this project already exists", ErrCode.ConflictingConnection, -1, {
                existingConnection: existing.getProvisionerDetails()
            });
        }

        // Try to set up a webhook
        // Requires at least a "Maintainer" role: https://docs.OpenProject.com/ee/user/permissions.html
        let warning: ConnectionWarning | undefined;
        if (OpenProjectConfig.webhook.publicUrl && permissionLevel >= AccessLevel.Maintainer) {
            const hooks = await client.projects.hooks.list(project.id);
            const hasHook = hooks.find(h => h.url === OpenProjectConfig.webhook.publicUrl);
            if (!hasHook) {
                log.info(`Creating webhook for ${validData.path}`);
                await client.projects.hooks.add(project.id, {
                    url: OpenProjectConfig.webhook.publicUrl,
                    token: OpenProjectConfig.webhook.secret,
        ...
        } else if (!OpenProjectConfig.webhook.publicUrl) {
        ...
        await new OpenProjectGrantChecker(as, OpenProjectConfig, tokenStore).grantConnection(roomId, { instance: validData.instance, path: validData.path })
    ...
    public static async getBase64Avatar(avatarUrl: string, client: OpenProjectClient, storage: IBridgeStorageProvider): Promise<string|null> {
        try {
            ...
            if (axios.isAxiosError(ex)) {
                if (ex.response?.status === 401) {
                    // 401 means that the project is Private and OpenProject haven't fixed
                    // the auth issues, just ignore this one.
                    // https://OpenProject.com/OpenProject-org/OpenProject/-/issues/25498
                    return null;
                }
            }
    ...
    public static async getConnectionTargets(userId: string, config: BridgeConfigOpenProject, filters: OpenProjectTargetFilter = {}, tokenStore: UserTokenStore, storage: IBridgeStorageProvider): Promise<OpenProjectConnectionTarget[]> {
        // Search for all repos under the user's control.

        if (!filters.instance) {
            const results: OpenProjectConnectionInstanceTarget[] = [];
            for (const [name, instance] of Object.entries(config.instances)) {
                const client = await tokenStore.getOpenProjectForUser(userId, instance.url);
                if (client) {
                    results.push({
                        name,
                    });
        ...
        const client = instanceUrl && await tokenStore.getOpenProjectForUser(userId, instanceUrl);
        if (!client) {
        ...
        }))) as OpenProjectConnectionProjectTarget[];
    }
    ...
    constructor(
        ...
        stateKey: string,
        as: Appservice,
        config: BridgeConfigOpenProject,
        private readonly intent: Intent,
        state: ConnectionStateValidated,
        private readonly tokenStore: UserTokenStore,
        private readonly instance: OpenProjectInstance,
    ...
    /**
     * The project's path string as returned by OpenProject,
    ...
    protected validateConnectionState(content: unknown) {
        return OpenProjectConnection.validateState(content);
    }
    ...
    public async onStateUpdate(stateEv: MatrixEvent<unknown>) {
        const validatedState = OpenProjectConnection.validateState(stateEv.content);
    ...
    private async getClientForUser(userId: string) {
        const client = await this.tokenStore.getOpenProjectForUser(userId, this.instance.url);
        if (!client) {
            throw new CommandError('User is not logged into OpenProject', 'You must be logged in to create an issue.');
        }
        return client;
    }
    ...
    private validateMREvent(event: IOpenProjectWebhookMREvent) {
    ...
    public async onMergeRequestOpened(event: IOpenProjectWebhookMREvent) {
    ...
    public async onMergeRequestReopened(event: IOpenProjectWebhookMREvent) {
    ...
    public async onMergeRequestClosed(event: IOpenProjectWebhookMREvent) {
    ...
    public async onMergeRequestMerged(event: IOpenProjectWebhookMREvent) {
    ...
    public async onMergeRequestUpdate(event: IOpenProjectWebhookMREvent) {
    ...
    public async onOpenProjectTagPush(event: IOpenProjectWebhookTagPushEvent) {
        if (this.hookFilter.shouldSkip('tag_push')) {
            return;
        }
        log.info(`onOpenProjectTagPush ${this.roomId} ${this.instance.url}/${this.path} ${event.ref}`);
    ...
    public async onOpenProjectPush(event: IOpenProjectWebhookPushEvent) {
        if (this.hookFilter.shouldSkip('push')) {
            return;
        }
        log.info(`onOpenProjectPush ${this.roomId} ${this.instance.url}/${this.path} ${event.after}`);
        ...
        const shouldName = !event.commits.every((c: { author: { email: any; }; }) => c.author.email === event.user_email);
        ...
        const commits = event.commits.reverse().slice(0,displayedCommits).map((commit: { id: string | any[]; title: any; author: { name: any; }; }) => {
    ...
    public async onWikiPageEvent(data: IOpenProjectWebhookWikiPageEvent) {
    ...
    public async onRelease(data: IOpenProjectWebhookReleaseEvent) {
...
${data.description}`;
    ...
    private async renderDebouncedMergeRequest(uniqueId: string, mergeRequest: IOpenProjectMergeRequest, project: IOpenProjectProject) {
        ...
        void this.persistDiscussionThreads().catch(ex => {
            log.error(`Failed to persistently store OpenProject discussion threads for connection ${this.connectionId}:`, ex);
        });
    }

    private debounceMergeRequestReview(
        user: IOpenProjectUser,
        mergeRequest: IOpenProjectMergeRequest,
    ...
    public async onMergeRequestReviewed(event: IOpenProjectWebhookMREvent) {
    ...
    public async onMergeRequestIndividualReview(event: IOpenProjectWebhookMREvent) {
    ...
    public async onCommentCreated(event: IOpenProjectWebhookNoteEvent) {
    ...
    public toString() {
        return `OpenProject ${this.instance.url}/${this.path}`;
    }
    ...
    public async provisionerUpdateConfig(userId: string, config: Record<string, unknown>) {
        ...
        const validatedConfig = OpenProjectConnection.validateState(config);
    ...
    public async onRemove() {
        ...
        try {
            ...
            await this.intent.underlyingClient.getRoomStateEvent(this.roomId, OpenProjectConnection.LegacyCanonicalEventType, this.stateKey);
            await this.intent.underlyingClient.sendStateEvent(this.roomId, OpenProjectConnection.LegacyCanonicalEventType, this.stateKey, { disabled: true });
        }
        // TODO: Clean up webhooks
    }

    private setDiscussionThreads(discussionThreads: SerializedOpenProjectDiscussionThreads): void {
    ...
    private async persistDiscussionThreads(): Promise<void> {
        const serialized: SerializedOpenProjectDiscussionThreads = [];
        ...
        return this.storage.setOpenProjectDiscussionThreads(this.connectionId, serialized);
    }
}
